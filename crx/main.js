import { c as createWebHashHistory, a as createRouter, E as ElNotification, b as createStore, d as createBlock, o as openBlock, r as resolveComponent, e as createApp, i as installer } from "./vendor.js";
...
axios$2.Axios = Axios;
axios$2.CanceledError = CanceledError_1;
  ...
  get: async (url, data2, headers) => {
    ...
    return new Promise((resolve, reject) => {
      axios$1({
        ...
        ErrorHandler(data22.data);
        resolve(data22.data);
      }, reject);
    });
  },
  post: async (url, data2, headers) => {
    await getToken(defaultHeaders);
    return new Promise((resolve, reject) => {
      axios$1({
        url: BASE_URL + url,
...
async function login(data2) {
  return await axios.post("/qj/v1/auth/token/login/email", data2);
}
async function getJobs(data2) {
  return await axios.get("/admin/v1/job/list", data2);
}
...
app.use(installer, { size: "mini" }).use(store).use(router).use(monitor).mount("#app");
